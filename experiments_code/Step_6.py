...
def evaluate_model_regularCV(X_train_standarized, Y_train, folds):
    ...
    # evaluate model
    scores = cross_val_score(model, X_train_standarized, Y_train, cv=cv, scoring=rmse_scoring, n_jobs=-1)
    return scores

model = LinearRegression()
ideal = cross_val_score(model, X_train_standarized, Y_train, cv=LeaveOneOut(), scoring=rmse_scoring, n_jobs=-1)

meanIdeal=mean(ideal)
print('Ideal: %.7f' % meanIdeal)
sensitivity_folds = range(2 ,41)
# record mean and min/max of each set of results
means, mins, maxs, results_regularCV, ideal_distances, std_errors = list(),list(),list(), list(), list(), list()
# evaluate each k value
for k in sensitivity_folds:
    # define the test condition
    cv = KFold(n_splits=k, shuffle=True, random_state=RANDOM_STATE)
    # evaluate k value
    sensitivity_scores = evaluate_model_regularCV(X_train_standarized, Y_train, k)
    #store scores
    ...
    # report performance
    print('> folds=%d, ideal_diff=%.8f, accuracy=%.8f (%.8f,%.8f), stderr=%.8f ' % (k, difference, meanValue, sensitivity_scores.min(),sensitivity_scores.max(), std_error))
...
print('Best related to ideal: %.8f at position: %d for fold: %d' % (min_ideal_difference, min_ideal_difference_index, sensitivity_folds[min_ideal_difference_index]))
print('Std Error value for the best accuracy score: %.8f' % (std_errors[min_ideal_difference_index]))

print('Best std error value %.8f at position: %d for fold: %d' % (min_std_error, min_std_error_index, sensitivity_folds[min_std_error_index]))
print('Worst std error value %.8f at position: %d for fold: %d' % (max_std_error,max_std_error_index, sensitivity_folds[max_std_error_index]))
...
pyplot.boxplot(results_regularCV, labels=[str(r) for r in sensitivity_folds], showmeans=True, meanprops=meanlineprops, meanline=True)
# show the plot
pyplot.show()
...
# record mean and min/max of each set of results
means2, mins2, maxs2, results_repeatedCV, ideal_distances2, std_errors2, folds_repeats_verbal_legend = list(),list(),list(), list(), list(), list(), list()
...
print('Std Error value for the best accuracy score: %.8f' % (std_errors2[min_ideal_difference_index2]))

print('Best std error value %.8f at position (fold & repeat): %s' % (min_std_error2, folds_repeats_verbal_legend[min_std_error_index2]))
print('Worst std error value %.8f at position (fold & repeat): %s' % (max_std_error2, folds_repeats_verbal_legend[max_std_error_index2]))
...
meanpointprops = dict(marker='D', markeredgecolor='black', markerfacecolor='firebrick')
meanlineprops = dict(linestyle='solid', linewidth=2.5, color='blue')
pyplot.boxplot(results_repeatedCV, labels=[str(r) for r in folds_repeats_verbal_legend], showmeans=True, meanprops=meanlineprops, meanline=True)
...
model_names = []
for name, model in models:
    cv_ideal_results = cross_val_score(model, X_train_standarized, Y_train, cv=rkfold, scoring=rmse_scoring, n_jobs=-1)
...
# label the plot
pyplot.xlabel('Mean Accuracy (Repeated CV)', fontsize=16, fontweight='bold')
pyplot.ylabel('Mean Accuracy (LOOCV)', fontsize=16, fontweight='bold')
# show the plot
pyplot.show()
...
print(f"Execution duration for standard cross validation sensitivity analysis: {time_end_sensitivity_analysis_cv - time_start_sensitivity_analysis_cv:0.4f} seconds")
print(f"Execution duration for repeated cross validation sensitivity analysis: {time_end_sensitivity_analysis_rcv - time_start_sensitivity_analysis_rcv:0.4f} seconds")
print(f"Execution duration for standard cross validation on algorithms: {time_end_cv - time_start_cv:0.4f} seconds")
print(f"Execution duration repeated cross validation on algorithms: {time_end_rcv - time_start_rcv:0.4f} seconds")